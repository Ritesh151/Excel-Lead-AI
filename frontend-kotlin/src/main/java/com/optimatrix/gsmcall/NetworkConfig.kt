 ...
 *   ✓ Use LAN IP address (e.g., 10.75.233.119), NOT localhost/127.0.0.1
 *   ✓ Use HTTP for development (configured in network_security_config.xml)
 *   ✓ Backend must be running on 0.0.0.0:3000
 *
 * Paths:
 *   HTTP:  http://10.75.233.119:3000/health
 *   WS:    ws://10.75.233.119:3000/socket.io/
 *   API:   http://10.75.233.119:3000/api/...
 */
object NetworkConfig {
    ...
    // Path: /socket.io/ is the Socket.IO handshake endpoint
    ...
    init {
        if (host.contains("localhost") || host.contains("127.0.0.1")) {
            android.util.Log.w(
                ...
                "Use LAN IP instead (e.g., 10.75.233.119)"
            )
        }
    }
